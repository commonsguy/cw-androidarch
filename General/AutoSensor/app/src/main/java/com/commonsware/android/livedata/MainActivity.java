...
public class MainActivity extends SimpleLifecycleActivity {
  ...
  public void onCreate(Bundle savedInstanceState) {
    ...
    state.sensorLiveData.observe(this, new Observer<AutoSensorEvent>() {
      @Override
      public void onChanged(@Nullable AutoSensorEvent event) {
        adapter.add(event);
      }
    });
  }
  ...
  private static class State {
    final ArrayList<AutoSensorEvent> events=new ArrayList<>();
  ...
  private static class RowHolder extends RecyclerView.ViewHolder {
    ...
    void bind(AutoSensorEvent event) {
      tv.setText(String.format("%s = %f", fmt.format(new Date(event.date())),
        event.values().get(0)));
    }
  }
}